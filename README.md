...
| 指標 | 値 |
|---|---|
| テスト | **1860 PASS / 0 FAILED** |
| REST API | **127 エンドポイント**（+8 v3.0 追加）|
...
| **v2.8.0** | **マルチフォーマット エクスポーター / セッション管理 / ComfyUI 22ノード** |
| **v2.9.0** | **日本語→タグ翻訳 / プロンプトチェーン / ComfyUI API クライアント** |
| **v3.0.0** | **プラグインマーケットプレイス / Gradio アダプター / PWA / レート制限** |

---

## v3.0.0 — プラットフォーム化 🎉

v3.0 は v2.x の「単一ツール」から「**拡張可能なプラットフォーム**」への転換点です。

### ① プラグインマーケットプレイス（fps-core/marketplace/ 新設）

```bash
# マーケットプレイスを閲覧・検索
GET /marketplace?q=anime&type=dictionary

# プラグインをインストール（IDで指定）
POST /marketplace/install  {"plugin_id": "novelai-quality-tags"}

# URL から直接インストール（GitHub/任意URL）
POST /marketplace/install  {"url": "https://github.com/.../dict.json", "plugin_type": "dictionary"}

# インストール済み一覧
GET /marketplace/installed

# アンインストール
DELETE /marketplace/installed/{plugin_id}
```

**初期収録プラグイン（5種）:**
- `novelai-quality-tags` — NovelAI 品質タグ辞書（⭐4.7）
- `anime-character-presets` — アニメキャラクタープリセット集 50種（⭐4.5）
- `photorealistic-rules` — 写実スタイルルールセット（⭐4.3）
- `landscape-wildcards` — 風景 Wildcard パック（⭐4.2）
- `jp-style-templates` — 日本語スタイルテンプレート集（⭐4.4）

### ② Gradio アダプター（fps-adapters/gradio/ 新設）

```bash
# スタンドアロン起動（ポート 7861）
python fps-adapters/gradio/app.py

# A1111 WebUI Extension として起動
# extensions/ にシンボリックリンクを作成 → 再起動で "FPS" タブ出現
```

Gradio UI の機能:
- **コンパイル** — Positive/Negative プロンプトをコンパイル
- **日本語→タグ変換** — TranslateEngine をそのまま使用
- **Wildcard 展開** — テンプレートをインタラクティブに展開

### ③ REST API 強化（ミドルウェア + +8本）

**CORS ミドルウェア:** ComfyUI(8188) / A1111(7860) / Gradio(7861) からのリクエストを許可

**レート制限ミドルウェア:**
- 匿名: 100 req/分
- 認証済み（X-Api-Key）: 1000 req/分
- 超過時: HTTP 429 + `Retry-After` ヘッダー

```
GET  /marketplace          プラグイン検索・閲覧
GET  /marketplace/installed インストール済み一覧
POST /marketplace/install   インストール（ID / URL）
DELETE /marketplace/installed/{id} アンインストール
GET  /health/detailed       全コンポーネント詳細ヘルスチェック
GET  /rate-limit/status     レート制限設定確認
POST /compile/async         非同期バッチコンパイル（202 Accepted）
```

### ④ PWA 化 — スマートフォンでインストール可能

`manifest.json` + `sw.js` の追加で:
- **ホーム画面にインストール** — iOS/Android/Chrome
- **オフライン対応** — Service Worker による静的アセットキャッシュ
- **レスポンシブ CSS** — モバイル画面でもナビが横スクロール対応

### ⑤ Web UI Marketplace タブ（15タブ目）
...
### ① 日本語→タグ翻訳エンジン（fps-core/translate/ 新設）

```bash
POST /translate/jp-to-tags
{"text": "青い目の金髪の少女、笑顔、アニメ風"}
→ {"tags":["blue_eyes","blonde_hair","1girl","smile","anime_style"],
   "confidence":0.83, "unmapped":[], "method":"builtin"}
```

- **外部API不要** — 内蔵変換テーブル（100+単語）+ 日本語辞書 1002件
- **部分一致対応** — 「金髪の少女」→「金髪」+「少女」を個別に変換
- **外部翻訳API 連携オプション**（LibreTranslate/DeepL 互換）

### ② プロンプトチェーン（fps-core/chain/ 新設）
...
# チェーン実行
POST /chains/{id}/run
{"prompt":"青い目の少女、__style__"}
→ {"final_pos":"blue_eyes, 1girl, anime_style, masterpiece, ...", "steps":[...]}
```

### ③ ComfyUI HTTP API クライアント（fps-adapters/comfyui_api/ 新設）

```bash
# 接続状態確認
GET /comfyui-client/status
→ {"available":true,"queue_remaining":2,"system_stats":{...}}

# プロンプトを ComfyUI に直接送信
POST /comfyui-client/queue
{"pos":"masterpiece, blue_eyes","steps":20,"cfg":7}
→ {"prompt_id":"abc123","status":"pending"}
```

### ④ 🎭 Face Prompt Translate ノード（ComfyUI 23ノード目）

```
[日本語テキスト: "青い目の金髪の少女"]
        ↓
[🎭 Face Prompt Translate]
  translated_tags → "blue_eyes, blonde_hair, 1girl"
  merged_prompt   → 既存タグ + 変換タグ
        ↓
[🎭 Face Prompt Compiler] → KSampler
```

### ⑤ Web UI 強化

- **Editor タブ**: 日本語入力ボックス → 変換タグをクリックして挿入
- **Dashboard タブ**: ComfyUI 接続パネル（状態確認・直接送信）

---

## v2.8.0 新機能

### ① マルチフォーマット エクスポーター（fps-core/export/ 新設）

```bash
# A1111 WebUI 互換形式
POST /export/a1111
{"pos":"masterpiece, blue_eyes","neg":"bad_quality","steps":20,"cfg":7}
→ "masterpiece, blue_eyes\nNegative prompt: bad_quality\nSteps: 20, ..."

# NovelAI 互換形式（(tag:1.2) → {{tag}} 自動変換）
POST /export/novelai
→ {"input":"masterpiece, {{blue_eyes}}", "parameters":{...}}

# 全形式を ZIP で一括エクスポート
POST /export/bundle
→ fps_export_bundle.zip (a1111.txt / novelai.json / fps.json / prompt.yaml / prompts.csv)
```

### ② プロンプトセッション管理（fps-core/session/ 新設）

```bash
# セッション作成
POST /sessions  {"name": "アニメキャラ比較"}

# エントリを蓄積
POST /sessions/{id}/entries  {"pos":"...", "label":"variant_A", "score": 82.0}
POST /sessions/{id}/entries  {"pos":"...", "label":"variant_B", "score": 87.5}

# エントリ間の差分比較
GET /sessions/{id}/compare/0/1
→ {"only_in_a":["soft_light"],"only_in_b":["golden_hour"],"score_diff":5.5}
```
...
| ノード | 概要 |
|---|---|
| 🎭 Face Prompt Export | pos/neg を A1111/NovelAI/JSON/YAML に変換して出力 |
| 🎭 Face Prompt Session Save | プロンプトをセッションに蓄積（ComfyUI内から） |
| 🎭 Face Prompt Session Load | セッションのベストエントリを取得 |

**バリエーション蓄積ワークフロー:**
```
[プロンプトテンプレート] → [🎭 Wildcard] → [🎭 Compiler]
                                               prompt_out ↓     score ↓
                                          [🎭 Session Save]  ← 複数回実行
                                               ↓
                                          [🎭 Session Load]  → best_pos → KSampler
```

### ④ Web UI 強化（13タブ）

- **Session タブ**: セッション一覧・エントリ蓄積・差分比較・ベスト使用
- **Editor タブ**: 「📤 Export」ボタン追加（コンパイル後に即エクスポート可）
- **エクスポートダイアログ**: A1111/NovelAI/JSON/YAML 形式選択 + ZIP Bundle 一括
